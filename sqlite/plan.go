...
type PlanModel struct {
	...
	ots.PlanStatusTimestamps `gorm:"embedded,embeddedPrefix:timestamp_"`
}
...
func NewPlanFromModel(model *PlanModel) *ots.Plan {
	return &ots.Plan{
		ID:                   model.ExternalID,
...
func (s PlanService) GetPlan(id string) (*ots.Plan, error) {
...
func (s PlanService) UpdatePlanStatus(id string, status tfe.PlanStatus) (*ots.Plan, error) {
	model, err := getPlanByID(s.DB, id)
	if err != nil {
		return nil, err
	}
...
func (s PlanService) FinishPlan(id string, opts *ots.PlanFinishOptions) (*ots.Plan, error) {
	model, err := getPlanByID(s.DB, id)
	if err != nil {
		return nil, err
	}
