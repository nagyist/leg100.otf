...
const updateWorkspaceCurrentRunByIDSQL = `UPDATE workspaces
SET
    current_run_id = $1
WHERE workspace_id = $2
RETURNING workspace_id;`

// UpdateWorkspaceCurrentRunByID implements Querier.UpdateWorkspaceCurrentRunByID.
func (q *DBQuerier) UpdateWorkspaceCurrentRunByID(ctx context.Context, currentRunID pgtype.Text, id pgtype.Text) (pgtype.Text, error) {
	ctx = context.WithValue(ctx, "pggen_query_name", "UpdateWorkspaceCurrentRunByID")
	row := q.conn.QueryRow(ctx, updateWorkspaceCurrentRunByIDSQL, currentRunID, id)
	var item pgtype.Text
	if err := row.Scan(&item); err != nil {
		return item, fmt.Errorf("query UpdateWorkspaceCurrentRunByID: %w", err)
	}
	return item, nil
}

// UpdateWorkspaceCurrentRunByIDBatch implements Querier.UpdateWorkspaceCurrentRunByIDBatch.
func (q *DBQuerier) UpdateWorkspaceCurrentRunByIDBatch(batch genericBatch, currentRunID pgtype.Text, id pgtype.Text) {
	batch.Queue(updateWorkspaceCurrentRunByIDSQL, currentRunID, id)
}

// UpdateWorkspaceCurrentRunByIDScan implements Querier.UpdateWorkspaceCurrentRunByIDScan.
func (q *DBQuerier) UpdateWorkspaceCurrentRunByIDScan(results pgx.BatchResults) (pgtype.Text, error) {
	row := results.QueryRow()
	var item pgtype.Text
	if err := row.Scan(&item); err != nil {
		return item, fmt.Errorf("scan UpdateWorkspaceCurrentRunByIDBatch row: %w", err)
	}
	return item, nil
}
